...
def calc_gravitational_potential_energy(mass, rho):
    G = 1.0 #(this is in code units)
    M = sum(mass)
    rho_sum = np.sum(rho)
    rho_avg = rho_sum / len(mass) #Should it be len(mass) here?
    R = (3.0*M / (4.0*np.pi*rho_avg))**(1.0/3.0)
    #get gravitational potential energy assuming spherical, constant density cloud
    E = 3.0/5.0*G*M**2.0/R * uenergy #converting to cgs here
    return E
...
#Output: mach_numbers (average, sd of the distribution, min and max) in unitless quantities
def calc_mach_numbers(vel, cs):
    speed = np.sqrt(vel[:,0]**2.0 + vel[:,1]**2.0 + vel[:,2]**2.0)
    mach = (speed*uvel) / (cs*10.0**5)
    mach_avg = np.mean(mach)
    mach_std = np.std(mach)
    mach_min = min(mach)
    mach_max = max(mach)
    mach_num = np.array([mach_avg, mach_std, mach_min, mach_max])
    return mach_num

#Input: vel in codeunits, cs in km/s 
#Output: mach_number in unitless quantity
def calc_mach_numbers_v2(vel, cs):
